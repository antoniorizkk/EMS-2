...
import { addEmployee, getEmployees } from "@/lib/helper"

export default function AddEmployeeForm({formData,setFormData}){
    ...
    const handleSubmit = (e) => {
        ...
        if (!firstname || !lastname || !email || !salary || !date || !status) {
            console.log("Missing fields detected");
            return <Bug message={"Please fill in all required fields"}></Bug>;
        }
        const model = {
    ...
    if (addMutation.isSuccess) return <Success message="Employee Added"></Success>;
    return(
        <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
            ...
            <div className="flex gap-10 items-center">
                <div className="form-check">
                    <input type="radio"  onChange={handleStatusChange} value="Active" id="radioDefault1" name="status" className="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"/>
                    <label htmlFor="radioDefault1" className="inline-block text-gray-800 ">
                        Active
                    </label>
                </div>
                <div className="form-check">
                    <input type="radio" onChange={handleStatusChange} value="Inactive" id="radioDefault2" name="status" className="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"/>
